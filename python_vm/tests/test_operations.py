...
@pytest.mark.parametrize(
    "op,curr_idx,input_bin,next_idx,res_bin",
    [
        ('add', 0, [9, 1, 4, 3], 4, [9, 7, 2, 3]),
        ('add', 0, [9, 1, 32758, 15], 4, [9, 5, 2, 3]),
        ('mul', 0, [10, 1, 5, 4], 4, [10, 20, 5, 4]),
        ('mul', 0, [10, 1, 8192, 5], 4, [10, 8192, 8192, 5]),
...
@pytest.mark.parametrize(
    "op,curr_idx,input_bin,next_idx,res_bin",
    [
        ('and', 0, [12, 1, 2, 3], 4, [12, 2, 2, 3]),
        ('or', 0, [13, 1, 3, 4], 4, [12, 7, 3, 4]),
        ('not', 0, [14, 1, 100], 3, [14, 32667, 100])
    ]
)
def test_bitwise_ops(sefault_ops_object, op, curr_idx, input_bin, next_idx, res_bin):
    operation = get_operation(default_ops_object, op, input_bin)
    result = operation(curr_idx)
    assert result == next_idx
    assert input_bin == res_bin
...
def test_mem_manip_ops(sefault_ops_object, op, curr_idx, input_bin, next_idx, res_bin):
    operation = get_operation(default_ops_object, op, input_bin)
    result = operation(curr_idx)
    assert result == next_idx
    assert input_bin == res_bin
...
@pytest.mark.parametrize(
    "op,curr_idx,input_bin,next_idx,side_effect",
    [
        ('out', 0, [19, 97], 2, 'mocked_print'),
        pytest.param('in', 0, [20, 10], 2,
                     'mocked_input', marks=pytest.mark.skip)
    ]
)
def test_io_ops(default_ops_object, op, curr_idx, input_bin, next_idx, side_effect):
    operation = get_operation(default_ops_object, op, input_bin)
    result = operation(curr_idx)
    (eval(side_effect)).assert_called_once()
    assert result == next_idx
